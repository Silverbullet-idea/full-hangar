-- Rebuild filter-options RPC to scan aircraft_listings (indexed is_active) instead of public_listings
-- (view does to_jsonb per row — too slow for full-table aggregates and hits REST statement timeouts).

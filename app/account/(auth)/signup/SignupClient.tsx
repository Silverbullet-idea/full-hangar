    ...
    try {
      ...
      const { error: err } = await supabase.auth.signUp({
        ...
        options: {
          ...
          emailRedirectTo: typeof window !== "undefined" ? `${window.location.origin}/account/auth/callback` : undefined,
        },
      })
      if (err) {
  ...
  return (
    <div className="w-full max-w-[400px] rounded-xl border border-[#30363d] bg-[#161b22] p-6 shadow-xl [data-theme=light]:border-slate-200 [data-theme=light]:bg-white">
      ...
      <form onSubmit={(e) => void onSubmit(e)} className="flex flex-col gap-3">
        <div>
          <label className="mb-1 block text-xs font-medium text-[#8b949e]">Display name (optional)</label>
          <input
            type="text"
      ...
      <p className="mt-6 text-center text-sm text-[#8b949e] [data-theme=light]:text-slate-600">
        ...
        <Link href={loginHref} className="font-semibold text-[#FF9900] no-underline hover:underline">
          Sign in →
        </Link>
      </p>

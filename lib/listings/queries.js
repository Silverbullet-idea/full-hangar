...
async function fetchRows(path, options = {}) {
  ...
  const authToken = resolveSupabaseRequestToken()
  if (!url || !authToken) {
    throw new Error("Missing Supabase URL/token for listings queries")
  }
...
function resolveServiceRoleKeyFromScraperEnv() {
...
function resolveSupabaseRequestToken() {
  return (

-- Filter-options RPC: tier counts + score bands use flip_tier / flip_score.
...
WITH active AS MATERIALIZED (
  SELECT
    ...
    NULLIF(upper(btrim(al.flip_tier)), '') AS flip_tier,
    CASE
      WHEN al.flip_score IS NULL THEN NULL
      WHEN btrim(al.flip_score::text) ~ '^-?\d+(\.\d+)?$' THEN btrim(al.flip_score::text)::numeric
      ELSE NULL
    END AS flip_score
...
counts AS (
  SELECT
    ...
    (SELECT count(*)::int FROM norm WHERE flip_score IS NOT NULL AND flip_score >= 60) AS score60,
    (SELECT count(*)::int FROM norm WHERE flip_score IS NOT NULL AND flip_score >= 80) AS score80
)
SELECT jsonb_build_object(
  ...
  jsonb_build_object(
    ...
    'TOP_DEALS',
    (SELECT count(*)::int FROM norm WHERE flip_tier IN ('HOT', 'GOOD')),
    'HOT',
    (SELECT count(*)::int FROM norm WHERE flip_tier = 'HOT'),
    'GOOD',
    (SELECT count(*)::int FROM norm WHERE flip_tier = 'GOOD'),
    'FAIR',
    (SELECT count(*)::int FROM norm WHERE flip_tier = 'FAIR'),
    'PASS',
    (SELECT count(*)::int FROM norm WHERE flip_tier = 'PASS')
  ),

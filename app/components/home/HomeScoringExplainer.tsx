...
export default function HomeScoringExplainer({ sampleListingHref }: HomeScoringExplainerProps) {
  return (
    <section id="how-we-score" className="mt-14 rounded-2xl border border-[#3A4454] bg-[#121923] p-6 md:p-9">
      <div className="mb-7">
        <p className="text-xs font-semibold uppercase tracking-[0.14em] text-[#8fa7c4]">Scoring framework</p>
        <h2 className="mt-2 text-3xl font-extrabold leading-tight text-white md:text-4xl">How Full Hangar scores aircraft</h2>
        <p className="mt-3 max-w-3xl text-sm text-[#c6d0dd] md:text-base">A simple 0-100 score to highlight buying edge.</p>
      </div>
      <div className="grid gap-4 md:grid-cols-3">
        {PILLARS.map((pillar) => (
          <article key={pillar.title} className="rounded-xl border border-[#3A4454] bg-[#161f2d] p-5 shadow-[0_10px_24px_rgba(0,0,0,0.2)]">
            <div className="text-xs font-semibold uppercase tracking-wide text-[#9bb0c9]">{pillar.weight} weight</div>
            <h3 className="mt-1 text-xl font-bold text-[#FF9900]">{pillar.title}</h3>
            <p className="mt-3 text-sm leading-relaxed text-[#d2dae6]">{pillar.description}</p>
          </article>
        ))}
      </div>
      <div className="mt-6 grid gap-4 md:grid-cols-2">
        <div className="rounded-xl border border-[#3A4454] bg-[#161f2d] p-5">
          ...
          <p className="mt-2 text-sm leading-relaxed text-[#d2dae6]">Data Confidence and Pricing Confidence show how reliable each score is.</p>
        </div>
        <div className="rounded-xl border border-[#3A4454] bg-[#161f2d] p-5">
          <h3 className="text-lg font-bold text-white">Transparent factors</h3>
          <p className="mt-2 text-sm leading-relaxed text-[#d2dae6]">See the exact factors behind every score on the detail page.</p>
          <Link

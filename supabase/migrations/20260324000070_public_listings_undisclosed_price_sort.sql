-- Browse sort: disclosed-price listings first, undisclosed/null/$0 last (Phase 2 UI overhaul).
ALTER TABLE public.aircraft_listings
ADD COLUMN IF NOT EXISTS undisclosed_price_sort smallint
GENERATED ALWAYS AS (
  CASE
    WHEN asking_price IS NULL OR COALESCE(asking_price, 0) <= 0 THEN 1::smallint
    ELSE 0::smallint
  END
) STORED;

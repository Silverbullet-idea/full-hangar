...
- [x] Non-aircraft workflow upgraded to hybrid policy: `scraper/audit_non_aircraft_listings.py` now outputs separate strict auto-hide and borderline manual-review files (`scraper/non_aircraft_strict_candidates_latest.json`, `scraper/non_aircraft_borderline_review_latest.json`) plus combined compatibility output (`scraper/non_aircraft_review_latest.json`) with reason-level confidence tags; `scraper/hide_listings_by_id.py` now supports `--input-mode strict-report`; added npm aliases (`pipeline:audit:non-aircraft:hybrid`, `pipeline:hide:non-aircraft:strict:dry`, `pipeline:hide:non-aircraft:strict:apply`); latest hybrid audit scan (`active_rows=4405`) reports `strict=94`, `borderline=41`, and confirms `source_id=5016588051` in strict bucket
- [x] Executed strict non-aircraft hide apply pass (`pipeline:hide:non-aircraft:strict:apply`): updated 94 rows (`failed=0`), immediate dry-run confirmed `would_hide=0` and `already_hidden=94`, and post-apply audit now shows `strict=0` / `borderline=41` with target `source_id=5016588051` marked inactive (`is_active=false`, `inactive_date=2026-03-07`)

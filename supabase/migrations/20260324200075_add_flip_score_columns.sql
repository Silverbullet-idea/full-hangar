-- flip_score / flip_tier / flip_explanation (intelligence v2.0.0); expose on public_listings view.
...
CREATE INDEX IF NOT EXISTS idx_aircraft_listings_flip_tier
  ON public.aircraft_listings (flip_tier)
...
SELECT
  ...
  CASE
    WHEN NULLIF(j ->> 'flip_score', '') ~ '^-?\d+(\.\d+)?$' THEN (j ->> 'flip_score')::integer
    ELSE NULL
  END AS flip_score,
  UPPER(NULLIF(j ->> 'flip_tier', '')) AS flip_tier,
  CASE
    WHEN jsonb_typeof(j -> 'flip_explanation') = 'object' THEN j -> 'flip_explanation'
    ELSE NULL::jsonb
  END AS flip_explanation
FROM base
WHERE

"""
bridge_server_globalair.py
Full Hangar - Local Bridge Server for GlobalAir Browser Extension Harvester

Receives listing data from the GlobalAir browser extension content script and feeds
...
LOG_FILE = LOG_DIR / "bridge_server_globalair.log"
CHECKPOINT_FILE = STATE_DIR / "bridge_checkpoint_globalair.json"
BATCH_SIZE = 20
UNSUPPORTED_DB_COLUMNS = {
    # Current Supabase project does not expose this column in aircraft_listings.
    "interior_notes",
}

LOG = logging.getLogger("bridge_server_globalair")
...
def normalize_and_validate_listing(raw: dict[str, Any], existing_source_ids: set[str], today_iso: str) -> tuple[dict[str, Any] | None, str | None]:
    row = dict(raw or {})
    row["source_site"] = str(row.get("source_site") or SOURCE_SITE).strip() or SOURCE_SITE
    ...
    normalized = {
        ...
        if not str(k).startswith("_") and str(k) not in UNSUPPORTED_DB_COLUMNS
    }
...
def make_handler(*, supabase: Any, state: BridgeState):
    class BridgeRequestHandler(BaseHTTPRequestHandler):
        server_version = "FullHangarGlobalAirBridge/1.0"
        ...
        def do_GET(self) -> None:  # noqa: N802
            if self.path == "/ping":
                self._write_json(200, {"status": "ok", "source_site": SOURCE_SITE})
                return
            if self.path == "/status":
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Full Hangar GlobalAir local bridge server for extension ingestion")
...
def main() -> None:
    args = parse_args()
    setup_logging()
    LOG.info("Starting GlobalAir bridge server on 127.0.0.1:%s (dry_run=%s)", args.port, args.dry_run)

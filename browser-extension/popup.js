...
function addLog(msg) {
  ...
  logEl.innerHTML = logLines
    .map(
      (l) =>
        `<div class="log-line ${l.includes('ERROR') ? 'error' : l.includes('Complete') ? 'success' : ''}">${l}</div>`
    )
    .join('');
}
...
function updateUI(state, bridgeAlive) {
  ...
  label.textContent = bridgeAlive ? 'Bridge running on :8765' : 'Bridge offline - start bridge_server.py';
  ...
  let statusText = 'Idle';
  if (state.running) {
    statusText = `Running: ${state.currentMake || ''} p.${state.currentPage || 1} | Queue: ${(state.listingQueue || []).length}`;
...
document.getElementById('btn-start').addEventListener('click', async () => {
  const makesRaw = document.getElementById('makes-input').value.trim();

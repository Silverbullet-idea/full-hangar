  ...
  return (
    <section className="mt-14 grid gap-6 lg:grid-cols-[1.2fr_1fr]">
        ...
        <div className="mt-5 grid gap-3">
          {FLOW_STEPS.map((item) => (
            <article key={item.title} className="rounded-xl border border-[#3A4454] bg-[#111a27] p-5 shadow-[0_8px_20px_rgba(0,0,0,0.2)]">
              <div className="text-xs font-semibold uppercase tracking-wide text-brand-muted">{item.step}</div>
      ...
      <aside className="rounded-2xl border border-[#3A4454] bg-[#121923] p-6 md:p-7">
        ...
        <p className="mt-3 text-sm text-brand-muted">High-signal patterns that often indicate actionable pricing edge.</p>
        <ul className="mt-4 space-y-3">
          {SIGNALS.map((signal) => (
            <li key={signal} className="rounded-lg border border-[#3A4454] bg-[#161f2d] px-3 py-2.5 text-sm text-brand-muted">
              {signal}
            </li>
          ))}
        </ul>

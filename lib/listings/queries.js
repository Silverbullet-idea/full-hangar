...
export async function getSiblingListingsByNNumber(nNumber, options = {}) {
  const normalized = normalizeNNumber(nNumber)
  if (!normalized) return []

  const excludeSourceId = typeof options.excludeSourceId === "string" ? options.excludeSourceId.trim() : ""
  const excludeId = typeof options.excludeId === "string" ? options.excludeId.trim() : ""
  const limit = Number.isFinite(Number(options.limit))
    ? Math.max(1, Math.min(25, Number(options.limit)))
    : 10

  const candidates = Array.from(
    new Set([
      normalized,
      normalized.startsWith("N") ? normalized.slice(1) : `N${normalized}`,
    ])
  ).filter(Boolean)
  if (!candidates.length) return []

  const query = new URLSearchParams()
  query.set("select", "id,source,source_id,listing_url,url,asking_price,price_asking,is_active,last_seen_date,n_number")
  query.set("is_active", "eq.true")
  query.set("limit", String(limit + 3))
  query.set("order", "last_seen_date.desc.nullslast")
  query.set(
    "or",
    candidates.map((candidate) => `n_number.eq.${encodeTerm(candidate)}`).join(",")
  )

  const rows = await fetchRows(`/rest/v1/${TABLE_NAME}?${query.toString()}`, { cache: "no-store" })
  return (rows || []).filter((row) => {
    const rowSourceId = typeof row?.source_id === "string" ? row.source_id.trim() : ""
    const rowId = typeof row?.id === "string" ? row.id.trim() : ""
    if (excludeSourceId && rowSourceId === excludeSourceId) return false
...
function normalizeNNumber(input) {
  if (typeof input !== "string") return ""
  const cleaned = input.replace(/[^A-Za-z0-9]/g, "").toUpperCase()
  if (!cleaned) return ""
  if (cleaned.startsWith("N")) return cleaned
  return `N${cleaned}`
}

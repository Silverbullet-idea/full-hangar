...
- UI overhaul Phase 1 shipped: sticky blurred global nav (`SiteHeader`) with logo + Barlow Condensed wordmark + BETA badge, centered 360px search (DM Sans, filter shortcut to `/listings`), live count pill via existing `GET /api/listings`, Deal Desk + Market Intel quick actions, `next/font` (Barlow Condensed / DM Sans / DM Mono), extended `--fh-*` design tokens + SVG grain on `body::before`, light-theme nav surface parity.
...
- ThemeToggle + HeaderBrand hydration mismatch fixed via mounted guard pattern (`ready` state + `suppressHydrationWarning` on the toggle shell); stable SSR/first-paint markup matches, eliminating logo `src` / `aria-*` drift and `BAILOUT_TO_CLIENT_SIDE_RENDERING` on header-bearing pages.
...
- Admin portal Phase 4B–4F refresh: `KpiCard`, Overview KPIs + value-score distribution + activity pulse, Listings grouped source KPIs + field-completeness heatmap (`source_stats`), Avionics segment rollout bars + BAS/Global priced donut + unresolved table actions, Engine tab backed by `computeEngineIntelligence()` and `GET /api/internal/admin/engine-intelligence`.
- Admin Phase 4G–4H: Scoring tab (price/tier integrity banner, pillar matrix, tier cards, buyer feed links) + Users & Beta (demo traffic SVGs, beta KPIs, session table, inventory highlights via `admin_inventory_highlights`); `computeAdminAudienceMetrics()` + `GET /api/internal/admin/audience-metrics`.
- Deal Desk Phase 5 (initial): default **guided wizard** on `/internal/deal-desk/[listingId]` with 6-step track, aircraft context banner, dot-grid panel, sticky **Live P&L** column (lg+), Back/Next; `?classic=1` restores tabbed layout.
- Deal Desk Phase 5E: per-step `WizardStepShell` / `DealDeskWizardStep` chrome (pulsing `deal-desk-step-card`, combined financing + exit on wizard step 5), shared `.deal-desk-inp*` / `.deal-desk-range` field styles in `globals.css`, form controls migrated on `DealDeskPageClient.tsx`.
- Deal Desk Phase 5B (step track): done steps show ✓ + emerald styling, active step gets orange focus ring, future steps stay disabled until **Next** advances `maxWizardStep` (deep links `?step=` still expand the unlocked range); connector segments turn green past unlocked progress.
- Deal Desk Phase 5G: sticky **Live P&L** sidebar — Barlow/DM Mono typography plus **phased cash-flow sections** (📥 Acquisition / 🔧 Upgrades / 📅 Carrying & financing / 🎯 Exit) with dotted rows, subtotals, bridge to all-in basis + total cash out + net proceeds; **profit hero** bumps `scale-[1.04]` for 300ms on net-profit change.
- Deal Desk Phase 5F (wizard content): shared `DealDeskChip` / `DealDeskInsight` / `DealDeskQuestionBlock`; step 1 aircraft confirmation + listing links; step 2 offer vs ask, seller-motivation + deferred-maintenance insights, annual-inspection chips; step 3 upgrade-focus chips + steam-gauge avionics nudge; step 4 hold presets + carry insights; step 5 financing intent chips, exit target vs ask + median ask context, marketing-lane chips, brokerage presets (self / broker / dealer), stretch-exit warning; step 6 “analysis complete” verdict card + **Print / Save as PDF** (`window.print`) + auto-save note; **Coming up** teaser for steps 2–5. Seed extended with listing-intel fields plus `riskLevel`, `valueScore`, `avionicsScore`, `dealRating`, `evPctLifeRemaining`, `faaMatched` for Live P&L health checklist.
- Deal Desk Phase **5D accordion**: guided mode stacks all six steps as collapsible cards (`DealDeskWizardStep`): clickable header (chevron, Barlow titles, **DONE / IN PROGRESS / NEXT / LOCKED** badges), current step expanded, completed steps toggle open on header click, future unlocked steps jump on header click; bodies stay mounted (hidden when collapsed) so form state + **print** show all sections (`print:!block` + health `details` expanded in print CSS). Classic `?classic=1` tab layout unchanged.
- Deal Desk Live P&L **score drill-down**: `DealDeskHealthScoreDrilldown` under health checklist — `<details>` groups for pillar/component scores, comps & pricing (`comp_*`, `mispricing_zscore`, `pricing_confidence`), engine `score_data` (`ev_explanation`, SMOH/TBO/remaining, contribution), accident signals; link to `/listings/[id]`; seed populated from `public_listings` on `deal-desk/[listingId]/page.tsx`.
- **Phase 6 score suppression (no disclosed price):** `core/intelligence/aircraft_intelligence.py` sets `value_score` and `deal_tier` to `None`, prepends suppression line to `score_explanation`, recomputes `risk_level` from `condition_score` when price missing/zero; **`INTELLIGENCE_VERSION` → `1.9.4`** (no backfill run in-repo). Frontend: `ListingCard` row/compact/legacy layouts show **Price undisclosed** pill when `tileMeta.hasDisclosedPrice` is false; `ListingScoreHeroCards` + `/listings/[id]` force undisclosed hero when ask ≤ 0.
...
# `listings-smoke.spec.js` probes `GET /api/listings?page=1&pageSize=1` once; if it is not 200 or returns `error`, the API matrix is **skipped** (page HTML checks still run) so local runs without Supabase stay green. Same file includes **listing detail shell**: missing slug must return 200 + “Listing not found”. `listings-options-no-loop.spec.js` skips if `GET /listings` is non-OK.

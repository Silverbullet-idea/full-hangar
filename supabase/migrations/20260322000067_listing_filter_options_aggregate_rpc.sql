-- Fast aggregate for /listings filter dropdowns (replaces multi-chunk public_listings scans that hit statement_timeout).

CREATE OR REPLACE FUNCTION public._normalize_listing_source_key(src text)
RETURNS text
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT CASE
    WHEN src IS NULL OR btrim(src) = '' THEN 'unknown'
    WHEN lower(btrim(src)) IN ('tap', 'trade-a-plane', 'tradaplane') THEN 'trade-a-plane'
    WHEN lower(btrim(src)) = 'controller_cdp' THEN 'controller_cdp'
    WHEN lower(btrim(src)) IN ('controller', 'ctrl') THEN 'controller'
    WHEN lower(btrim(src)) LIKE 'controller\_%' ESCAPE '\' THEN 'controller'
    WHEN lower(btrim(src)) IN ('aerotrader', 'aero_trader') THEN 'aerotrader'
    WHEN lower(btrim(src)) IN ('aircraftforsale', 'aircraft_for_sale', 'afs') THEN 'aircraftforsale'
    WHEN lower(btrim(src)) = 'aso' THEN 'aso'
    WHEN lower(btrim(src)) IN ('globalair', 'global_air') THEN 'globalair'
    WHEN lower(btrim(src)) = 'barnstormers' THEN 'barnstormers'
    ELSE lower(btrim(src))
  END;
$$;
...
WITH active AS (
  SELECT
    NULLIF(btrim(pl.make), '') AS make,
    NULLIF(btrim(pl.model), '') AS model,
    NULLIF(upper(btrim(pl.location_state)), '') AS state,
    COALESCE(NULLIF(btrim(pl.source), ''), '') AS source_raw,
    NULLIF(upper(btrim(pl.deal_tier)), '') AS deal_tier,
    pl.value_score AS value_score
...
SELECT jsonb_build_object(
  'makes',
  (
    SELECT COALESCE(jsonb_agg(q.make ORDER BY q.make), '[]'::jsonb)
    FROM (SELECT DISTINCT make AS make FROM norm WHERE valid_make) q
  ),
  'models',
  (
    SELECT COALESCE(jsonb_agg(q.model ORDER BY q.model), '[]'::jsonb)
  ...
  (
    SELECT COALESCE(
      jsonb_agg(jsonb_build_object('make', q.make, 'model', q.model) ORDER BY q.make, q.model),
      '[]'::jsonb
    )
...
REVOKE ALL ON FUNCTION public._normalize_listing_source_key(text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_listing_filter_options_payload() TO service_role;
GRANT EXECUTE ON FUNCTION public._normalize_listing_source_key(text) TO service_role;

CREATE INDEX IF NOT EXISTS idx_aircraft_listings_active_id
  ON public.aircraft_listings (id)

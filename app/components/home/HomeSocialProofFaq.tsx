        ...
        <div className="mt-5 space-y-3">
          {TESTIMONIALS.map((item) => (
            <blockquote key={item.quote} className="rounded-xl border border-[#3A4454] bg-[#161f2d] p-5 shadow-[0_8px_20px_rgba(0,0,0,0.2)]">
              <p className="text-base leading-relaxed text-brand-muted">"{item.quote}"</p>
              <footer className="mt-2 text-xs font-semibold uppercase tracking-wide text-brand-muted">{item.author}</footer>
            </blockquote>
          ))}
        </div>
      </div>

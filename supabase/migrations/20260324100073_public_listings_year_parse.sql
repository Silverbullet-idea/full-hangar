-- Richer model-year extraction for public_listings: trim/decimals, embedded digits in year field,
-- then first 19xx/20xx in title when the year column is empty or unparsable (browse year filters).

CREATE OR REPLACE FUNCTION public.parse_listing_year(j jsonb)
RETURNS integer
LANGUAGE sql
IMMUTABLE
PARALLEL SAFE
AS $$
  SELECT CASE
    WHEN y BETWEEN 1900 AND 2038 THEN y
    ELSE NULL
  END
  FROM (
    SELECT COALESCE(
      CASE
        WHEN btrim(COALESCE(j ->> 'year', '')) = '' THEN NULL::integer
        WHEN regexp_replace(btrim(COALESCE(j ->> 'year', '')), '\.+[0-9]*$', '') ~ '^\d{4}$'
          THEN regexp_replace(btrim(COALESCE(j ->> 'year', '')), '\.+[0-9]*$', '')::integer
        WHEN (regexp_match(btrim(COALESCE(j ->> 'year', '')), '(19[0-9]{2}|20[0-9]{2})')) IS NOT NULL
          THEN (regexp_match(btrim(COALESCE(j ->> 'year', '')), '(19[0-9]{2}|20[0-9]{2})'))[1]::integer
        ELSE NULL
      END,
      CASE
        WHEN (regexp_match(COALESCE(j ->> 'title', ''), '(19[0-9]{2}|20[0-9]{2})')) IS NOT NULL
          THEN (regexp_match(COALESCE(j ->> 'title', ''), '(19[0-9]{2}|20[0-9]{2})'))[1]::integer
        ELSE NULL
      END
    ) AS y
  ) s;
...
GRANT EXECUTE ON FUNCTION public.parse_listing_year(jsonb) TO anon, authenticated;
